...
public class ServerChatManager {
    ...
    public boolean register(ServiceChat user){
        ...
        if (status){
            try{
                getClients().getOutput().sendFormattedMessage(ServiceChat.SERVER_NAME,"\"%s\" is now connected",user.getUser().getName());
    ...
    public boolean remove(ServiceChat user){
        ...
        if (infos==null){
            ...
            if (status){
                try{
                    getClients().getOutput().sendFormattedMessage(ServiceChat.SERVER_NAME,"\"%s\" has disconnected",user.getUser().getName());
                }catch(PacketChatException e){
